		...
		<div class="card bg-base-200/50 border border-base-300">
			<div class="card-body p-6 space-y-4">
				...
				{:else if searchResults.length > 0}
					<div class="space-y-2">
						<label class="field-label text-sm" for="quickstart-search-results"
							>{$t('adventures.search_results')}</label
						>
						{#if searchProvider}
								...
								<div class="text-xs text-warning mt-2 inline-flex items-center gap-1">
									...
									{#if selectedLocation.review_count}
										<span class="text-base-content/60"
											>({selectedLocation.review_count} reviews)</span
										>
									{/if}
								</div>
							{/if}
				...
				<div class="card bg-base-100 border border-base-300">
					<div class="card-body p-4">
						<div class="flex flex-col gap-2">
							...
							/>
							<p class="field-hint">Optional. If not selected, backend defaults to General.</p>
						</div>
					</div>
				</div>
			{/if}
